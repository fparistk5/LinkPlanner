...
-- Function to check profile count per NFT
CREATE OR REPLACE FUNCTION check_profile_limit_per_nft()
RETURNS TRIGGER AS $$
BEGIN
  -- Only check if nft_token_id is provided (including TechKeyz profile)
  IF NEW.nft_token_id IS NOT NULL THEN
    -- Count existing profiles for this NFT
    -- For TechKeyz profile (token_id = '1'), count all profiles regardless of wallet
    ...
    IF (
      ...
      AND (
        -- For TechKeyz profile (token '1'), count all profiles
        (NEW.nft_token_id = '1') OR
        -- For other NFTs, only count profiles for this wallet
        (wallet_address = NEW.wallet_address)
      )
      AND id != COALESCE(NEW.id, -1) -- Exclude current record for updates
    ) >= 3 THEN
      RAISE EXCEPTION 'Maximum limit of 3 profiles per NFT exceeded';
    END IF;
  END IF;
...
CREATE INDEX IF NOT EXISTS idx_network_profiles_wallet_nft 
ON network_profiles(wallet_address, nft_token_id) 
